...
### Workstation arithmetic revision (2026-09-15)
The four-root searchable catalog contains 61 executable modules: the previous 50 plus five CUT and six Diffusion modules. Linear enters full arithmetic immediately: inputs, weighted products, dot sums, centered bias and output share a complete weighted connection fabric. Other operations show function-specific diagrams and actual operands in the main inter-layer corridor. Tensor coordinates remain complete, with exact factor rectangles for leading planes and long vectors. The stage operator index starts collapsed and pages six entries at a time. Models over 100 operations initially focus a named stage, with every other stage and full graph retrievable.
