...
The four-root searchable catalog contains 61 executable modules: the previous 50 plus five CUT and six Diffusion modules. Dense MLP input, hidden and output neurons and every weight edge remain visible. Linear and activation source layers are preserved; activation functions show aligned per-coordinate transfer traces with actual formulas and moving crystal units. All dense connections retain baseline visibility while smooth overlapping Gaussian highlights stream across neighboring outputs and terms. Numerical weights, full scalar coordinates and batches remain inspectable. Convolution retains concrete streamed contributions and partial sums, persistent links, a main trunk, nearby parameters and input-docked zero padding.

MLP/Linear have a quick parameter rail above the canvas and in fullscreen for Batch, input/output features and MLP hidden width/layer count; existing sidebar controls remain. The camera stays upright. Long-graph following, full-graph fitting and mathematical navigation remain available, as does persistent explanatory prose editing independent of live values. Verification covers 61 modules, 124 configurations, all MLP weights for Batch 1/2 and ten activation transfer functions. The earlier 4,241-stage count describes available expanded execution verification rather than the default visible scene. This revision is local and has not been deployed.
