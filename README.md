...
Workstation tensors use one uniform crystal per actual scalar. Overview, group, layer and operator views share a concrete arithmetic expansion; every source operator in the current scope remains present. Large contractions stream exact per-output contributions and partial sums while preserving all original tensor coordinates. Large configurations are rejected explicitly (65,536 scalars per tensor; 500,000 stored scalars per run). Dimensions, seed, training/eval behavior and input patterns persist in the browser. Custom JSON inputs must exactly match the configured shape. Exports preserve the computation's numerical records.
...
Every scope, including the overview, renders concrete mathematical stages from a shared execution graph. Linear exposes all weighted products, dot sums, bias and output. Activations include their arithmetic intermediates and GELU erf/Horner stages; normalization exposes statistics, centering and scaling. Convolution and reductions stream exact contributions and all partial sums for each output, while pooling exposes running comparisons. Dropout, Box–Muller, L2 normalization, time encoding and loss operations also expose concrete stages. Dense edges remain dim context, with the current output and term highlighted from the same trace as the live equation.

The catalog follows four roots → task family → module. The operator index starts collapsed and pages six entries per stage. Graphs deeper than 24 levels follow the current operator with the camera while retaining the full graph. Mathematical step navigation, a follow toggle and full-graph fitting support long executions. Inputs and constants sit near their consumers. The top editing toolbar provides persistent editing of explanatory prose independently of live values.

Tensor planes follow their last two dimensions; preceding coordinates address a tiled plane array. Leading planes and long vectors use exact factor rectangles (48 becomes 6 × 8), preserving all coordinates without empty corners. All cells have uniform size. Cyan marks nonnegative values, amber negative values and pale white the selection; signed edges use a workstation-specific optional style. Parallel branches use dependency depth, and long skip edges route outside a vertical longest-dependency trunk with local spacing. The implementation adapts ideas from TensorSpace's [LayerLocator](https://github.com/tensorspace-team/tensorspace/blob/master/src/utils/LayerLocator.js), [FeatureMap](https://github.com/tensorspace-team/tensorspace/blob/master/src/elements/FeatureMap.js), and [Conv2d](https://github.com/tensorspace-team/tensorspace/blob/master/src/layer/intermediate/Conv2d.js), inspected on 2026-09-14, using the project's current Three.js runtime and crystal primitives.
...
Current local verification: 61 models, 124 configurations, 3,149 PyTorch-compared outputs and 4,241 expanded mathematical stages across default configurations. The concrete-arithmetic revision has not been deployed.
