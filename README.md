...
Linear always opens directly into full arithmetic: every input connects through weighted products and all dot sums to centered bias and output. W is encoded in connection colors and retained completely in the data inspector. Other modules expose principal tensor scaffolds, with operation-specific diagrams and actual operand links in the main inter-layer corridor. Detached miniature calculators have been removed. The catalog follows four roots → task family → module. The operator index starts collapsed and pages six entries per stage. Models over 100 operations initially focus a named stage; other stages and the full graph remain retrievable without a global web crossing the focused view. Normalization statistics and stable Softmax reductions unfold inside their owning layer.
...
Complex labels appear on hover or explicit reveal, with KaTeX in Linear labels and full formulas below. Camera controls include cursor-targeted wheel zoom, plus/minus, pan/rotate, double-click tensor framing, reset and fullscreen.
