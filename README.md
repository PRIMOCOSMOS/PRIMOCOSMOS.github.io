...
The six literature viewers compile their existing audited source entries into `SourceModule` compositions and render them through `ModuleAssembly`. The first view displays immediate functional children, real dependency edges and shortcut paths; deeper compositions and existing numerical operator visuals load only for the active function. Static source geometry is batched, and inactive numerical visuals are disposed. Hover selects a visible child or cycles the children of a containing function. Navigation resets hover ownership so a previous source ID cannot stall the new local scene. MMHVAE clears the overview's active numerical visual on entry to a detail view.

Source dimensions and mathematical demonstrations remain distinct: frames represent source tensor axes/extents, while atomic animations use the declared small deterministic numerical examples. They do not execute trained paper checkpoints or stand in for full-resolution activation tensors. Source identities, repeated block instances, shared-weight notes, mathematical parameterizations and exact dimension labels remain in the existing source-audited catalogs. PIGMENT remains paper-derived where no official implementation is confirmed.
