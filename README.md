...
文献总览已恢复按功能区分的 MMHVAE 微缩造型及模态配色，并通过几何合批保持低绘制开销。数学视图共用工作站正负数值色标，未知值单独标示；在“张量数值”中下载当前坐标模板、填入实际数值并导入，即可为对应激活/权重着色。文件仅在浏览器内读取，不上传；切换页面后需重新导入。`npm run verify:paper-colors` 验证造型类别、模态颜色、合批、常量、数值导入校验和共享水晶色阶。
