 ...
 useEffect(()=>{
  ...
  const buildDetail=(graph:AnatomyGraph,id:string)=>{
   ...
   for(const p of graph.parts){
    ...
    const group=new T.Group(),glyph=makeGlyph(p.glyph,color);glyph.scale.setScalar(p.position?1.4:1.7);group.add(glyph)
    ...
    const dim=label(p.shape,'#b3cbd4',.48,5.4);dim.position.set(0,.8,0);dim.userData.baseScale=dim.scale.clone();dim.userData.part=p.id;group.add(dim)
   ...
   const frameBounds=bounds.clone().expandByPoint(new T.Vector3())
   ...
   frameBounds.getSize(detailExtent);detailCenter.copy(frameBounds.getCenter(new T.Vector3()).add(anchor));transition(fit(detailCenter,detailExtent,latest.current.detailZoom))
   renderer.domElement.dataset.envelope=JSON.stringify({min:layout.min,max:layout.max,ports:parts.filter(v=>v.part.role).map(v=>({id:v.part.id,position:v.dest.toArray()}))})
  ...
  const pick=(e:PointerEvent)=>{const r=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,1-(e.clientY-r.top)/r.height*2);ray.setFromCamera(pointer,camera);return ray.intersectObjects(detail?parts.flatMap(p=>[p.hit,...p.label.visible?[p.label]:[]]):[...visuals.values()].filter(v=>v.group.visible).map(v=>v.hit))[0]?.object.userData}
  ...
  const move=(e:PointerEvent)=>{if(e.buttons)return;const hit=pick(e);hoverPart=hit?.part??'';setHover(hit?.part?partName(parts.find(p=>p.part.id===hit.part)!.part):hit?.id?moduleName(NODE_MAP.get(hit.id)!):'');renderer.domElement.style.cursor=hit?'pointer':'grab';dirty=true}
  ...
  const animate=(now:number)=>{
   ...
   controls.update();if(dirty){
    // Resolve labels in screen space after every camera movement. Hover always wins.
    scene.updateMatrixWorld(true);camera.updateMatrixWorld(true)
    const occupied:{x:number;y:number;w:number;h:number}[]=[],w=container.clientWidth,h=container.clientHeight
    const candidates=parts.filter(v=>v.label.visible).sort((a,b)=>Number(b.part.id===(hoverPart||p.activePart))-Number(a.part.id===(hoverPart||p.activePart)))
    for(const v of candidates){const point=v.label.getWorldPosition(new T.Vector3()).project(camera),distance=camera.position.distanceTo(v.label.getWorldPosition(new T.Vector3())),unit=h/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2))*distance)*camera.zoom
     for(const tag of [v.label,v.dim]){const base=tag.userData.baseScale as T.Vector3;tag.scale.copy(base).multiplyScalar(Math.max(1,(tag===v.label?18:15)/(base.y*unit)))}
     const rect={x:(point.x+1)*w/2,y:(1-point.y)*h/2,w:v.label.scale.x*unit+10,h:v.label.scale.y*unit+8}
     const collision=occupied.some(r=>Math.abs(r.x-rect.x)<(r.w+rect.w)/2&&Math.abs(r.y-rect.y)<(r.h+rect.h)/2)
     v.label.visible=!collision&&point.z<1&&point.z>-1&&rect.x-rect.w/2>4&&rect.x+rect.w/2<w-4;if(v.label.visible)occupied.push(rect)
    }
...
renderer.domElement.dataset.camera=JSON.stringify([...camera.position.toArray(),...controls.target.toArray(),camera.zoom]);renderer.render(scene,camera);dirty=false}
 ...
 return <div className="mm-orbit-host" ref={host}>{error&&<div className="mm-webgl-fallback"><h4>当前浏览器无法显示三维场景</h4><p>下方模块目录与内部算子序列仍可查看完整结构、来源、去向和公式。</p><a href="#mm-module-directory">打开模块目录</a></div>}{hover&&!error&&<div className="mm-hover-label">{hover}<span>{props.detailGraph?'点击进入组件 · 边界外节点跳转到来源 / 去向':'点击原位拆解 · 镜头将移入模块'}</span></div>}</div>
}
