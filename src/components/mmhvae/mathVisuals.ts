...
import {connectionFabric} from '../workstation/operationMotion'
...
export function makeMathVisual(spec:MathStage):MathVisual{
 ...
 const tiles=(count:number,size=.42)=>createCrystalTensor(group,count,size,{valueEdges:true,valueScale:2,bodyOpacity:.18,edgeOpacity:.42}).update
 ...
 if(spec.kind==='convolution'||spec.kind==='depthwise'){
  ...
  const kernelSet=tiles(k*k,.3),kernelPos=gridPositions(k,1,Math.min(1.7,k*.42)).map(p=>[p[0]-3.3,p[1]+2.6,p[2]] as V),window=wireBox((k-1)*4.8/(n-1)+cellSize,.6,(k-1)*4.8/(n-1)+cellSize)
  const denseLinks=[],weightExtent=Math.max(...weights.map(Math.abs));for(let oi=0;oi<output.length;oi++){const oc=Math.floor(oi/(outN*outN)),localOutput=oi%(outN*outN),row=Math.floor(localOutput/outN)*spec.stride-spec.padding,col=localOutput%outN*spec.stride-spec.padding;for(let c=0;c<channels;c++){if(spec.kind==='depthwise'&&c!==oc)continue;for(let ky=0;ky<k;ky++)for(let kx=0;kx<k;kx++){let y=row+ky,x=col+kx;if(spec.reflect){y=y<0?-y:y>=n?2*n-2-y:y;x=x<0?-x:x>=n?2*n-2-x:x}if(y<0||x<0||y>=n||x>=n)continue;const wi=c*k*k+ky*k+kx;denseLinks.push({from:positions[c*n*n+y*n+x],to:outPositions[oi],weight:weights[wi],weightScale:weightExtent,output:oi,term:wi})}}}
  const convolutionFabric=connectionFabric(group,denseLinks);updates.push(f=>convolutionFabric.update(f.phase*2,f.probe*output.length,f.phase*k*k*channels,.85,.55));
  ...
  const n=6,m=4,x=demoValues.slice(0,n),rawW=Array.from({length:n*m},(_,i)=>Math.sin(i*1.4)*.55),W=spec.weightNorm?rawW.map((w,i)=>w/Math.hypot(...rawW.slice(Math.floor(i/n)*n,(Math.floor(i/n)+1)*n))):rawW
  const xp:V[]=x.map((_,i)=>[(i-2.5)*.85,2.2,0]),yp:V[]=Array.from({length:m},(_,i)=>[(i-1.5)*1.1,-2.2,0]),bp:V[]=yp.map(p=>[p[0],-1.35,.8]),input=tiles(n),output=tiles(m),bias=spec.bias?tiles(m,.3):undefined
  const y=Array.from({length:m},(_,i)=>x.reduce((s,a,j)=>s+a*W[i*n+j],spec.bias?.1:0))
  const fabric=connectionFabric(group,W.map((weight,i)=>({from:xp[i%n],to:yp[Math.floor(i/n)],weight,weightScale:Math.max(...W.map(Math.abs)),output:Math.floor(i/n),term:i%n})))
  if(bias)for(let i=0;i<m;i++)line([bp[i],yp[i]],'#cfaf84',.45)
  cursor.visible=false
  updates.push(f=>{const row=Math.min(m-1,Math.floor(f.probe*m)),col=Math.floor(f.phase*n)%n;group.userData.readout=`y_${row}=${y[row].toFixed(3)}`;input(x,xp,{focus:col,active:Array.from({length:n},(_,i)=>i)});output(y,yp,row);bias?.(Array(m).fill(.1),bp,row);fabric.update(f.phase,row,col);flow([xp[col],yp[row]],f.phase)})
  ...
  const a=demoValues.slice(0,16),b=Array.from({length:16},(_,i)=>spec.kind==='mask'?i%4===0?0:1:spec.kind==='multiplication'?.35+(i%4)*.15:demoValues[i+16]),out=a.map((v,i)=>spec.kind==='addition'?v+b[i]:spec.kind==='mask'?2*((v+1)/2*b[i])-1:v*b[i]),grid=gridPositions(4,1,2.9),left=grid.map(p=>[p[0]-2.1,1,p[2]] as V),right=grid.map(p=>[p[0]+2.1,.05,p[2]] as V),result=grid.map(p=>[p[0],-1,p[2]] as V),inputA=tiles(16,.65),inputB=tiles(16,.65),output=tiles(16,.65)
 ...
 group.userData.annotations=spec.kind==='convolution'||spec.kind==='depthwise'?[annotation('X \\;[C,H,W]',[.5,3,0]),annotation('W \\;[C,k,k]',[-3.3,3.5,0]),annotation('Y \\;[C_{out},H_{out},W_{out}]',[.5,-3.2,0])]:spec.kind==='interpolation'?[annotation('X \\;[4,4]',[0,2.4,0]),annotation('Y \\;[8,8]',[0,-2.7,0])]:['addition','multiplication','mask'].includes(spec.kind)?[annotation('A',[-2.1,2,0]),annotation(spec.kind==='mask'?'m':'B',[2.1,.8,0]),annotation('Y',[0,-2,0])]:spec.kind==='fusion'?[annotation(spec.operation==='normalize'?'w,\\;m':'w_p,\\;w_1,\\;w_2',[0,2,0]),annotation(spec.operation==='normalize'?'\\mu,\\;s':spec.operation==='weighted-mean'?'m':'w',[0,-2.3,1])]:spec.kind==='broadcast'?[annotation('g \\;[C,1,1]',[-4,2.5,0]),annotation('g_{c,h,w}=g_c',[0,2.8,0])]:[]
