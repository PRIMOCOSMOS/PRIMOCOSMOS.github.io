...
export function mathSpec(p:AnatomyPart,formula:string):MathSpec{
 ...
 case 'normalization':control='观察通道';steps=[step('每个样本、通道独立统计',R`\mu_{bc}=\frac1{HW}\sum_{h,w}X_{bchw}`,'高亮层片中的所有空间位置汇入一个均值；统计不跨样本，也不跨通道。'),step('中心化与方差',R`v_{bc}=\frac1{HW}\sum_{h,w}(X_{bchw}-\mu_{bc})^2`,'先减均值，再逐元素平方。等尺寸水晶块的色彩强度展示非负的偏差平方；对它们求平均得到总体方差（unbiased=False）。'),step('缩放至统一响应尺度',R`Y_{bchw}=\frac{X_{bchw}-\mu_{bc}}{\sqrt{v_{bc}+10^{-5}}}`,'加入 eps 防止除零。此模型默认没有 affine 参数、不维护 running stats；不是 BatchNorm。')];break
 case 'activation':control=operation.includes('clamp')?'输入值 x / a（−20 至 20）':'输入值 x（−4 至 4）';legend='横轴为输入 x，竖轴为输出 f(x)，纵深方向分隔独立通道；水晶标记对应当前输入和函数值，箭头连接它们的投影。';steps=[step('独立读取每个张量元素',R`x=X_{b,c,h,w}`,'非线性作用于每个元素，保留 batch、通道和空间索引。选取的输入值同步出现在曲线和结果上。'),step('沿真实函数曲线映射',activationFormula(operation),'曲线由当前算子的真实函数求值绘制。轨迹投影同时展示输入与输出；负半轴行为与 SiLU、ReLU 等算子各自一致。'),step('回写同一张量位置',R`Y_{b,c,h,w}=f(X_{b,c,h,w})`,'数值改变，张量尺寸不变。soft-clamp 保留平滑梯度；含 exp 的先验尺度头会把结果转为正标准差。')];break
 case 'pooling':control='观察通道';steps=[step('收集一个通道的空间响应',R`X_c\in\mathbb R^{H\times W}`,'每层为一个通道，保留通道之间的独立性。'),step('空间求和再除以像素数',R`u_c=\frac{\sum_{h,w}X_{c,h,w}}{HW}`,'当前通道的空间单元依次高亮，箭头汇入该通道的描述值；示例直接计算平均值。'),step('形成通道描述向量',R`u\in\mathbb R^{B\times C}`,'SE 随后的 Linear 使用每个通道的全局描述；这不是最大池化，也不混合不同通道。')];break
 case 'concatenation':control='观察拼接通道';steps=[step('两路特征保留空间对齐',R`A\in\mathbb R^{B\times C_a\times H\times W},\quad B\in\mathbb R^{B\times C_b\times H\times W}`,'左右两组层片分别表示生成条件和模态 skip，来自边界外的两路输入；示例分别使用正值与负值以便追踪。'),step('沿通道轴连接层片',R`Y=\operatorname{cat}(A,B;\mathrm{dim}=1)`,'层片沿竖直的通道轴排列；元素不做相加、不改变空间坐标。'),step('按通道区间寻址',R`Y_c=\begin{cases}A_c&c<C_a\\B_{c-C_a}&c\ge C_a\end{cases}`,'观察位置对应同一个元素在拼接前后的地址；新通道数为两路之和。')];break
 ...
 case 'fusion':control='示例专家的尺度';legend='同一排水晶块依次表示先验与专家的标量；冷蓝与暖色表示数值的非负与负值。源码使用倒数 scale 权重；此处只展示当前算子的计算。';steps=[step('读取先验与残差专家',R`w_p=1/s_p,\quad w_j=e^{-a_j}`,'固定先验示例 μp=0、sp=1；两个观测专家示例均值 −0.8 和 1.2。只累加当前可用模态。'),step('累加权重和均值贡献',R`w=w_p+\sum_jw_j,\quad m=w_p\mu_p+\sum_jw_j\mu_j`,'各通道分别计算。权重与贡献的水晶块来自相同示例数值；源码按 1/s，理论 Gaussian PoE 则按 1/s²。'),step('归一化并建立后验',R`\mu=m/w,\quad s=1/w,\quad q=\mathcal N(\mu,(Ts)^2)`,'改变一个专家的尺度，可观察其权重、融合中心和宽度同步变化。温度只缩放标准差。')];break
 ...
 case 'addition':case 'multiplication':case 'mask':
  control='观察元素位置';steps=[step(kind==='mask'?'影像与背景掩码':'两路对齐的操作数',kind==='mask'?R`m=\mathbf1[x_{\mathrm{first}}> -1]`:R`A,B\in\mathbb R^{C\times H\times W}`,'左右位置区分两路输入；箭头连接相同通道和空间位置。门控向量以 C×1×1 广播。'),step(kind==='addition'?'逐位置相加':kind==='mask'?'缩放后应用掩码':'逐位置相乘',kind==='addition'?operation==='sample-mean'?R`z_i=\mu_i+\eta_i`:R`Y_i=A_i+B_i`:kind==='mask'?R`Y_i=2((X_i+1)/2\cdot m_i)-1`:operation==='sample-scale'?R`\eta_i=Ts_i\epsilon_i`:R`Y_i=A_iB_i`,'展示实际示例操作数及计算结果，不跨位置求和。残差加法保留直接梯度路径；乘法以对应权重调节响应。'),step('回写与输入相同的结构',kind==='mask'?R`m_i=0\Rightarrow Y_i=-1,\quad m_i=1\Rightarrow Y_i=X_i`:R`\operatorname{shape}(Y)=\operatorname{shape}(A)`,'层片保持原有空间对齐；掩码为零时输出背景值 −1，而不是零。')];break
 ...
 if(kind==='fusion'){
  if(operation==='weight')steps=[step('读取尺度参数',R`(s_p,\{a_j\})`,'先验提供标准差，残差专家提供 log-scale。分布曲面仅作为操作数的来源语义，不在此处构建新后验。'),step('转换为倒数尺度权重',R`w_p=1/s_p,\quad w_j=\exp(-a_j)`,'等尺寸水晶块依次代表同一坐标的先验和专家权重。改变专家尺度时，色彩强度与数值读数显示倒数关系。'),step('逐坐标累加权重',R`w=w_p+\sum_jw_j`,'此算子输出总权重 w，不输出后验分布；均值累加、归一化和 Normal 分别属于其下游模块。')]
  if(operation==='weighted-mean')steps=[step('读取各分布均值与尺度',R`(\mu_p,s_p),\quad(\mu_j,a_j)`,'参数来自先验与观测专家。从左到右为先验与两个教学示例专家；颜色只表示数值正负。'),step('形成加权均值贡献',R`c_p=\mu_p/s_p,\quad c_j=\mu_j\exp(-a_j)`,'每个均值乘自己的倒数尺度；贡献可以为正或负，冷暖颜色区分符号，色彩强度与读数表示幅值；立方体大小保持统一。'),step('累加贡献，等待归一化',R`m=c_p+\sum_jc_j`,'输出未归一化的加权和 m，而不是最终均值。下一模块再计算 μ=m/w。')]
