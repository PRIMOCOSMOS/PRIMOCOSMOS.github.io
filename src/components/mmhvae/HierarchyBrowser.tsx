...
export default function HierarchyBrowser({id,observed,onNavigate,mathStep=0,onMathStep}:{id:string;observed:string[];onNavigate:(id:string)=>void;mathStep?:number;onMathStep?:(n:number)=>void}){
  ...
  return <section className="mm-browser" id="mm-module-directory" aria-label="模型层级资源管理器" data-location={id}>
    ...
    <div className="mm-browser-layout"><aside className="mm-browser-tree" aria-label="模型组成目录">{tree('root')}</aside><div className="mm-browser-content">
      ...
      {graph?.mathematics&&<div className="mm-browser-math"><h5>本算子的数学实现</h5><EditableText textKey={`mm-math-note-${id}`}>{graph.note}</EditableText><div className="mm-browser-files" aria-label="原子数学推导索引">{graph.mathematics.steps.map((s,i)=><button key={s.title} aria-pressed={mathStep===i} onClick={()=>onMathStep?.(i)}><span>{i+1}</span><span><strong>{s.title}</strong><small>{s.explanation}</small></span><ChevronRight size={15}/></button>)}</div><Equation value={graph.mathematics.steps[mathStep]?.formula??''}/></div>}
