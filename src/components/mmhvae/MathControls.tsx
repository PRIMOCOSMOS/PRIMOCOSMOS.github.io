...
export function MathControls({spec,selected,onSelect,probe,onProbe,progress,onProgress,playing,onPlaying,temperature,onOverview,textKeyBase}:{spec:MathSpec;selected:number;onSelect:(n:number)=>void;probe:number;onProbe:(v:number)=>void;progress:number;onProgress:(v:number)=>void;playing:boolean;onPlaying:()=>void;temperature:number;onOverview:()=>void;textKeyBase:string}){
