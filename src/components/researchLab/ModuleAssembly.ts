...
export function moduleAssembly(module:SourceModule,color:string){
 ...
 const draw=(node:SourceModule,x:number,y:number,z:number):number=>{
  if(!node.children.length){
   const position=new T.Vector3(x,y,z),staticGroup=sourceTensor(node.glyph,node.shape,color);staticGroup.position.copy(position);staticGroup.scale.set(5,2,3);structure.add(staticGroup);
 ...
 const height=draw(module,0,0,0);structure.position.y=0;
 ...
 const setActive=(id:string,frame:MathFrame)=>{
  ...
  if(active!==id){if(math){group.remove(math.group);disposeGroup(math.group);math=undefined}if(nested){group.remove(nested.group);disposeGroup(nested.group);nested=undefined}if(flow){group.remove(flow.group);disposeGroup(flow.group);flow=undefined}active=id;
   ...
   else if(cell?.module.expand){nested=moduleAssembly(cell.module.expand(),color);nested.group.scale.setScalar(8/nested.height);nested.group.position.copy(cell.position).add(new T.Vector3(0,(nested.height/2-5)*nested.group.scale.y,0));group.add(nested.group)}
  ...
  group.userData.activeFunction=cell?.module.id??'';group.userData.activeMath=Number(!!math)+(nested?.group.userData.activeMath??0);
  return cell;
 };
