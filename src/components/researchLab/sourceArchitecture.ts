...
export function paperModule(model:PaperModel,id:string,depth=2):SourceModule{
 ...
 return {id:e.id,title:e.title,shape:e.shape,dimensions,glyph:e.glyph,formula:e.formula,source:`${e.file}:${e.line} · ${e.symbol}`,operator:e.math,children:depth>0?e.children.map(id=>paperModule(model,id,depth-1)):[],edges:e.edges,expand:depth===0&&e.children.length?()=>paperModule(model,id,1):undefined};
}
