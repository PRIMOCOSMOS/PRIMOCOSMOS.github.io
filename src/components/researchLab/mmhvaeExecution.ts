...
export function concreteMmhvaePlan(observed:string[],temperature=.5):SourcePlan{
 ...
 const act=(r:string,id:string,x:string,type='silu')=>unary(r,id,type==='silu'?'SiLU':type==='relu'?'ReLU':type==='leaky'?'LeakyReLU(0.2)':type==='tanh'?'Tanh':'Sigmoid',x,type,type==='silu'?R`Y=X\sigma(X)`:type==='relu'?R`Y=\max(0,X)`:type==='leaky'?R`Y=\max(X,0.2X)`:type==='tanh'?R`Y=\tanh X`:R`Y=(1+e^{-X})^{-1}`);
 ...
 const se=(r:string,x:string)=>{
  ...
  let a=add(scope,'mean','每通道全局平均','pooling','pool',[c],[x],R`u_c=\frac1{HW}\sum_{h,w}X_{c,h,w}`);
  ...
  a=add(scope,'view','门控重排 C×1×1','reshape','reshape',[c,1,1],[a],R`g\leftarrow\operatorname{view}(g,C,1,1)`);
 ...
 for(const mod of observed){let x=add(`${mod}-input`,'input',`${MODALITIES.find(m=>m.id===mod)?.label} 输入切片`,'tensor','feature',[1,192,192],[],R`X\in\mathbb R^{B\times1\times192\times192}`);inputs.set(mod,x);x=conv(`${mod}-stem`,'first_conv',x,16);
 ...
 for(let l=7;l>=1;l--){const level=LEVELS.find(v=>v.l===l)!,shape=l===7?[256]:[level.channels,level.size,level.size];let feature='',muP='',scaleP='';
  ...
  for(const [j,e] of experts.entries()){const weight=unary(poe,`expert-weight-${j}`,'专家倒数尺度 exp(−a)',e.a,'exp-negative',R`w_j=\exp(-a_j)`),contribution=add(poe,`contribution-${j}`,'专家均值贡献','multiplication','multiply',shape,[e.mu,weight],R`m_j=\mu_jw_j`);w=add(poe,`weight-sum-${j}`,'累加倒数尺度','addition','add',shape,[w,weight],R`w\leftarrow w+w_j`);m=add(poe,`mean-sum-${j}`,'累加加权均值','addition','add',shape,[m,contribution],R`m\leftarrow m+m_j`)}
  ...
  const sample=`sample-${l}`,eps=add(sample,'eps','标准正态噪声 ε','tensor','normal-noise',shape,[],R`\epsilon\sim\mathcal N(0,I)`),noise=add(sample,'scale','标准差 × 噪声','multiplication','multiply',shape,[scale,eps],R`\eta=s_T\odot\epsilon`);z=add(sample,'mean',`潜变量 z${l} · 重参数化`,'addition','add',shape,[mu,noise],R`z=\mu+\eta`);
 ...
 for(const mod of MODALITIES){let x=z;
  ...
  const r=`${mod.id}-output`;x=conv(r,'convt2',x,8,7,1,3,true,false,false,true);x=norm(r,'norm2',x);x=act(r,'act',x,'leaky');x=conv(r,'convt3',x,1,7,1,3,true,false,false,true);x=act(r,'act_last',x,'tanh');add(`${mod.id}-image`,'mask','首个输入的背景掩码','mask','background',[1,192,192],[x,inputs.get(observed[0])!],R`Y=2((X+1)/2\cdot\mathbf1[X_{first}>-1])-1`);
 }
 return plan;
}
