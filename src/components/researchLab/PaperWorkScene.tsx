...
import PaperColorKey from './PaperColorKey'
import {applySourceValues,type SourceValues} from './sourceValues'
...
export default function PaperWorkScene(p:Props){
 ...
 const full=useMemo(()=>applySourceValues(p.model?makePaperRun(p.model,p.windowPage):makeMmhvaeRun(p.observed,p.windowPage,p.temperature),values?.model===(p.model?.id??'mmhvae')?values:undefined),[p.model,p.observed,p.windowPage,p.temperature,values]);
 ...
 return <div className="paper-work-scene" data-renderer="workstation" data-source-steps={full.steps.length} data-displayed-steps={run.steps.length} data-detail={location} data-lod={overview?'architecture':'mathematics'}>
  ...
  <div className="paper-coordinate-key"><span>{overview?`${run.steps.length} 个功能块 · ${full.steps.length} 个数学步骤`:`${run.steps.length} 个展开的数学步骤`}</span><span>{overview?'功能造型 · 分支配色 · 点击展开':'水晶 = 源码坐标 · 正负数值色阶'}</span><span>自动轮播 · 悬停接管</span></div>
  <PaperColorKey run={run} full={full} model={p.model?.id??'mmhvae'} values={values?.model===(p.model?.id??'mmhvae')?values:undefined} onValues={setValues}/>
