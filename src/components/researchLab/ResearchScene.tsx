...
interface Props {model:PaperModel;labelMode:'hover'|'all'|'none';graph:AnatomyGraph;selected:string;playing:boolean;reduced:boolean;probe:number;progress:number;step:number;focus:boolean;view:'orbit'|'front'|'top';reset:number;zoom:number;onSelect:(id:string)=>void;onStep:(n:number)=>void}
...
export default function ResearchScene(props:Props){
 ...
 useEffect(()=>{
  ...
  let graph:AnatomyGraph|null=null,sceneKey='',focusKey='',viewKey='',zoom=1,reset=-1,visible=true,hoverId='',raf=0,dirty=true,phase=0,lastTime=0,lastProgress=-1,lastMode='',activeNode='',hoverModule='',hoverClock=0
  ...
  const build=(g:AnatomyGraph)=>{
   ...
   for(const part of g.parts){
    ...
    const entry=latest.current.model.entries[part.child??part.id],assembly=!math&&!part.role&&entry?moduleAssembly(paperModule(latest.current.model,entry.id,1),color):undefined;const glyph=math?.group??assembly?.group??sourceTensor(part.glyph,part.shape,color);glyph.scale.setScalar(math?1:assembly?(g.layout==='overview'?4.8:8)/assembly.height:g.layout==='overview'?2.1:1.8);if(assembly)glyph.position.y=(assembly.height/2-5)*glyph.scale.y;item.add(glyph)
  ...
  const pointerMove=(e:PointerEvent)=>{if(e.buttons)return;const hit=pick(e),id=hit?.id??'',module=hit?.module??'';if(id!==hoverId||module!==hoverModule){hoverId=id;hoverModule=module;hoverClock=0;setHover(latest.current.model.entries[module]?.title??nodes.find(n=>n.part.id===id)?.part.title??'');dirty=true}}
