...
const recipes:Partial<Record<Op,Step[]>>={
 ...
 spectral:[['将核视作矩阵',R`W\in\mathbb R^{C_{out}\times(C_{in}k^2)}`,'光谱归一化约束权重矩阵的最大奇异值，不是权重向量 L2 归一化。'],['交替幂迭代',R`v\leftarrow W^Tu/\|W^Tu\|_2,\quad u\leftarrow Wv/\|Wv\|_2`,'u、v 交替更新估计主奇异方向；示例使用固定矩阵和12次迭代逼近；PyTorch 默认每次 forward 更新1次。'],['归一化谱尺度',R`\hat\sigma=u^TWv,\quad\bar W=W/\hat\sigma`,'训练时更新幂迭代缓冲；推理使用已有估计。不把它表示成额外的图像层。']],
...
export function operatorSpec(op:Op,detail='',formula=''):MathSpec{
 const standard:Partial<Record<Op,Glyph>>={tensor:'tensor',linear:'linear',conv:'conv',pool:'pool',reshape:'tensor',gaussian:'gaussian',sample:'sample',pad:'tensor',unpad:'tensor',broadcast:'tensor',tanh:'activation',add:'sum',multiply:'multiply',norm:'norm'}
 if(standard[op]){
  ...
  if(op==='linear'){spec.steps[2].explanation='各输出行分别求和。Linear 只变换特征维；空间重排由独立算子执行。';if(/bias=False/i.test(detail)){spec.bias=false;spec.steps[2]={title:'归约无偏置投影',formula:R`y_i=\sum_jq_{ij}`,explanation:'此投影显式关闭偏置。'}}}
  if(op==='gaussian')spec.steps[0].explanation='均值定位中心，标准差控制各独立坐标的宽度；对应当前模块提供的参数。'
  if(op==='pool')spec.steps[2].explanation='每个通道分别得到一个空间均值，通道之间不相互混合。'
  if(op==='tanh')spec.steps[2].explanation='将每个输入映射到 (−1,1)，空间和通道尺寸不变。'
  if(op==='add'||op==='multiply')spec.steps[2].explanation='对齐坐标写回结果，不跨位置归约。'
  return spec
 }
 const steps=op==='split'?[
 ['读取有序坐标',R`X\in\mathbb R^{B\times d}`,'先保留原始值与坐标，分组大小由当前模块指定。'],
 ['按指定区间分路',formula||R`(X_1,X_2)=(X[:,:k],X[:,k:])`,'切片或 chunk 只改变索引归属，不改变元素值。'],
 ['独立输出各组',R`X=\operatorname{Concat}(X_1,\ldots,X_r)`,'分路的颜色表示坐标归属；三组 QKV 或不等长组织/水分支按当前实现单独组织。']
 ]:op==='token-mean'?[
 ['读取真实梯度 token',R`H\in\mathbb R^{B\times G\times E}`,'CLS 已在上游剔除；每个位置仍对应扩散方向。'],
 ['沿梯度 token 轴求均值',R`f_{be}=\frac1G\sum_{g=1}^G H_{bge}`,'同一 embedding 坐标汇聚全部梯度方向，不平均 embedding 轴。'],
 ['生成受试者特征',R`f\in\mathbb R^{B\times1536}`,'推理脚本保存 fea[-1]。教学网格用4个方向、4个特征演示轴向归约。']
 ]:op==='bilinear'?[
 ['输出坐标对齐两端',R`u=u'\frac{H-1}{H'-1},\quad v=v'\frac{W-1}{W'-1}`,'align_corners=True；此判别器逐尺度下采样0.5。'],
 ['四邻点距离加权',R`Y=\sum_{i,j\in\{0,1\}}w_{ij}X_{ij},\quad\sum w_{ij}=1`,'根据反向映射坐标计算双线性权重，无可学习参数。'],
 ['形成下一尺度',R`[B,C,H,W]\to[B,C,\lfloor H/2\rfloor,\lfloor W/2\rfloor]`,'4×4缩小至2×2的教学例恰好采样四角，真实较大网格通常需要四点混合。']
 ]:op==='spatial-mask'?[
...
export function scientificExample(spec:MathStage,probe:number):NumericExample{
 if(spec.operation.startsWith('core:'))return coreExample(spec,probe)
 ...
 if(op==='token-mean')out=Array.from({length:4},(_,j)=>[0,1,2,3].reduce((s,i)=>s+a[i*4+j],0)/4)
 else if(op==='bilinear')out=[a[0],a[3],a[12],a[15]]
 else if(op==='split'){const groups=spec.operation.split(':')[2];a.splice(groups==='unequal'?7:groups==='3'?12:8);out=[...a]}
 else if(op==='leaky01'||op==='leaky02')out=a.map(x=>x<0?x*(op==='leaky01'?.01:.2):x)
 ...
 else if(op==='poe-precision'){const subset=spec.operation.split(':')[2],s=.45+probe*1.35,t1=subset==='two'?0:1/(s*s+1e-8),t2=subset==='one'?0:1/(.7**2+1e-8),tau=1+t1+t2,mu=(-.8*t1+1.2*t2)/tau;out=[mu,Math.sqrt(1/tau)];return {a:[0,-.8,1.2],b:[1,s,.7],out,index:0,readout:`μ = ${mu.toFixed(3)} · σ = ${out[1].toFixed(3)} · τ = ${tau.toFixed(3)}`}}
