...
export default function WorkScene(props:Props){const viewAction=useRef<(action:string,value?:number)=>void>(()=>{}),[following,setFollowing]=useState(false),[stageCount,setStageCount]=useState(1),[stageCursor,setStageCursor]=useState(0);const host=useRef<HTMLDivElement>(null),latest=useRef(props);latest.current=props;const [error,setError]=useState(''),[hover,setHover]=useState(''),[live,setLive]=useState({title:'',equation:'',step:''})
 useEffect(()=>{
  ...
  let lastLive=0;
  let coordinateKey='',coordinateMap=new Map<string,Set<number>>();let stageOrder=new Map<string,number>();let lastNotify=0,pass=0,execution:ReturnType<typeof executionGraph>,visibleSteps:Step[]=[]
  ...
  const field=(t:Tensor,origin:T.Vector3,select?:string,title?:string,formula?:string)=>{
   const group=new T.Group();group.position.copy(origin);body.add(group);const positions=tensorPositions(t),crystals=createCrystalTensor(group,t.values.length,.36,{valueEdges:true,bodyOpacity:.23,edgeOpacity:.5});crystals.update(t.values,positions,{focus:-1});const f={tensor:t,group,positions,crystals};fields.push(f);fieldMap.set(t.id,f)
  ...
  const linksFor=(step:Step,index:number,terms=step.trace(index)):Connection[]=>{const to=point(step.output.id,index);if(!to)return [];return terms.flatMap((t,j)=>{const from=point(t.tensor,t.index),weight=t.factorTensor?point(t.factorTensor,t.factorIndex!):undefined;return [...from?[{from,to,weight:t.factor??1,output:index,term:j}]:[],...weight?[{from:weight,to,weight:t.factor??1,output:index,term:j}]:[]]})};
  const rebuild=()=>{
   const p=latest.current;setHover('');setLive({title:'',equation:'',step:''});lastLive=-Infinity;manualFrame=undefined;fabrics=[];activeFabric=undefined;activeKey='';hoveredScope='';fieldBounds.clear();fieldMap.clear();scene.remove(body);disposeGroup(body);body=new T.Group();scene.add(body);labels.replaceChildren(leaders);leaders.replaceChildren();fields=[];clicks=[];tags=[];oldIndex=-1;pass=0;coordinateKey=''
   ...
   execution=executionGraph(p.run,selected);visibleSteps=execution.run.steps;stageOrder=new Map(visibleSteps.map((s,i)=>[s.id,i]));
   const graph=buildScaffold(execution.run,visibleSteps,true),stageView=p.scope!=='root'||!!p.focus;
   if(el.clientWidth<600&&graph.nodes.length<=24){let y=0;for(const n of [...graph.nodes].sort((a,b)=>a.level-b.level||Number(!!b.tensor.parameter)-Number(!!a.tensor.parameter))){n.position=[0,y,0];y-=Math.max(2.2,n.depth*.5+1.5)}}
   ...
   for(const n of graph.nodes){
    ...
    const name=n.step?n.step.title:n.tensor.parameter?`参数 · ${n.tensor.name}`:stageView?`阶段输入 · ${n.tensor.name}`:n.tensor.id===p.run.input.id?'输入 X':n.tensor.name;
    box.union(field(n.tensor,new T.Vector3(...n.position),destination,name,n.step?.formula));if(boundary)tags.at(-1)!.priority=100;
   }
   ...
   for(const block of execution.blocks){if(block.streamed)continue;for(const step of block.steps){let count=0;const links:Connection[]=[];for(let i=0;i<step.output.values.length;i++){const terms=step.trace(i);count+=terms.length;if(count>20000){links.length=0;break}links.push(...linksFor(step,i,terms))}if(links.length)fabrics.push({step,view:connectionFabric(body,links)})}}
   flow=createArrowStream(body,'#e8f8ff',.12,2);flow.group.visible=false;
   follow=graph.levels>24;framedOwner='';setFollowing(follow);setStageCount(visibleSteps.length);el.dataset.layers=String(graph.nodes.length);el.dataset.levels=String(graph.levels);el.dataset.mathStages=String(visibleSteps.length);
  ...
  const tick=(now:number)=>{raf=requestAnimationFrame(tick);const p=latest.current;if(run!==p.run||scope!==p.scope||layoutFocus!==p.focus){run=p.run;scope=p.scope;layoutFocus=p.focus;liveStep='';rebuild()}
   ...
   if(p.playing&&!reduced.matches){const next=phase+dt/Math.max(4,visibleSteps.length*1.1);if(next>=1){pass++;const atom=p.run.steps.find(s=>s.id===p.scope);if(atom)p.onIndex((p.index+1)%atom.output.values.length)}phase=next%1;dirty=true;if(now-lastNotify>120){lastNotify=now;p.onProgress(phase)}}
   ...
   if(active){
    ...
    block.select(outputIndex);liveStep=owner;if(follow&&!activeHighlight&&framedOwner!==owner){framedOwner=owner;frameOwner(owner)}
    ...
    activeFabric?.update(local,index,j,1);for(const item of fabrics){const distance=Math.abs(stageOrder.get(item.step.id)!-stageOrder.get(active.id)!);item.view.update(local,item.step===active?index:-2,j,item.step===active?.35:.065/Math.sqrt(1+distance))}
    ...
    if(now-lastLive>100){lastLive=now;setStageCursor(visibleSteps.indexOf(active));const format=(v:number)=>Number.isFinite(v)?v.toPrecision(4):String(v);setLive({title:active.title,equation:`输出 [${coords(outputIndex,block.source.output.shape).join(', ')}] · ${current?`读取 ${format(current.value)}${current.factor!==undefined?' × '+format(current.factor):''} → `:''}${format(active.output.values[index])}${block.streamed?' · 逐输出流式演算':''}`,step:owner})}
   }
