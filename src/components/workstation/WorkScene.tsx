...
export default function WorkScene(props:Props){const viewAction=useRef<(action:string,value?:number)=>void>(()=>{}),[following,setFollowing]=useState(false),[stageCount,setStageCount]=useState(1),[stageCursor,setStageCursor]=useState(0);const host=useRef<HTMLDivElement>(null),latest=useRef(props);latest.current=props;const [error,setError]=useState(''),[hover,setHover]=useState(''),[live,setLive]=useState({title:'',equation:'',step:''})
 useEffect(()=>{
  ...
  const field=(t:Tensor,origin:T.Vector3,select?:string,title?:string,formula?:string)=>{
   ...
   const b=t.window?new T.Box3().setFromPoints(positions.map(p=>new T.Vector3(...p).add(origin))).expandByScalar(latest.current.run.sourceGraph?.overview?1.4:.22):new T.Box3().setFromObject(group);fieldBounds.set(t.id,b.clone());
  ...
  const rebuild=()=>{
   ...
   if(p.run.sourceGraph){
    ...
    sourcePool=createCrystalTensor(body,positions.length,p.run.sourceGraph?.overview?2.6:.36,{valueEdges:true,bodyOpacity:.18,edgeOpacity:.5});sourcePool.update(values,positions,-1);sourceHighlights=createCrystalTensor(body,4096,.38,{valueEdges:true,bodyOpacity:.65,edgeOpacity:.95});
