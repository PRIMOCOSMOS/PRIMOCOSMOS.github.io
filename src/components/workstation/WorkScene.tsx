...
export default function WorkScene(props:Props){const host=useRef<HTMLDivElement>(null),latest=useRef(props);latest.current=props;const [error,setError]=useState(''),[hover,setHover]=useState(''),[live,setLive]=useState({title:'',equation:'',step:''})
 useEffect(()=>{
  ...
  let body=new T.Group(),fields:Field[]=[],clicks:T.Object3D[]=[],tags:{el:HTMLButtonElement;point:T.Vector3;line:SVGLineElement;priority:number;scope:string}[]=[],raf=0,run:Run|undefined,scope='',oldReset=-1,oldFocus='',oldHighlight='',hoveredScope='',oldZoom=0,oldPan=false,oldLabels=false,oldIndex=-1,oldProgress=-1,phase=0,last=0,dirty=true,visible=true,center=new T.Vector3(),extent=new T.Vector3(20,20,20)
  let lastLive=0;
  ...
  const fieldBounds=new Map<string,T.Box3>();let manualFrame:T.Box3|undefined,micro:ReturnType<typeof createCrystalTensor>|undefined,microGroup:T.Group|undefined,microFlow:ReturnType<typeof createArrowStream>|undefined,microInput:ReturnType<typeof createArrowStream>|undefined,microOutput:ReturnType<typeof createArrowStream>|undefined;
  ...
  const fit=()=>{const overview=latest.current.scope==='root'&&!latest.current.focus;camera.up.set(overview&&camera.aspect>1?1.15:0,1,0).normalize();
   ...
   const boxes=manualFrame?[manualFrame]:[...fieldBounds].filter(([id])=>!focused||focused.has(id)||id==='live-calculation').map(([,b])=>b),corners:T.Vector3[]=[];
  ...
  const field=(t:Tensor,origin:T.Vector3,select?:string,title?:string)=>{
   ...
   label(`${title??t.name}${layout.wrapped?' · 连续索引折行':''}\n[${t.shape.join(' × ')}]`,new T.Vector3(b.max.x+.35,origin.y+.4,origin.z),select??latest.current.scope,t.id===latest.current.run.input.id||t.id===(latest.current.run.steps.find(s=>s.id===latest.current.scope)?.output.id??latest.current.run.output.id)?100:0);return b
  }
  const rebuild=()=>{
   const p=latest.current;setHover('');setLive({title:'',equation:'',step:''});lastLive=-Infinity;manualFrame=undefined;micro=undefined;microGroup=undefined;microFlow=undefined;microInput=undefined;microOutput=undefined;hoveredScope='';fieldBounds.clear();scene.remove(body);disposeGroup(body);body=new T.Group();scene.add(body);labels.replaceChildren(leaders);leaders.replaceChildren();fields=[];clicks=[];tags=[];flow=undefined;factorFlow=undefined;traceLines=undefined;traceKey='';oldIndex=-1;connections=[];products=undefined;prefix=undefined;sums=undefined
   ...
   if(p.scope==='root'||p.scope.startsWith('group:')||p.scope.startsWith('layer:')){
    ...
    for(const e of graph.edges){const a=map.get(e.from)!,b=map.get(e.to)!;
     ...
     const arrow=createArrowStream(body,e.skip?'#e3c392':'#a2d4e4',.15,3);arrow.update(points,.4);arrow.group.userData.path=points;arrow.group.userData.to=e.to;arrow.group.userData.skip=e.skip;connections.push(arrow);
    }
    el.dataset.layers=String(graph.nodes.length);el.dataset.levels=String(graph.levels);
    // A live scalar calculation beside the trunk stays in the same 3D scene.
    microGroup=new T.Group();microGroup.position.set(box.max.x+3,box.getCenter(new T.Vector3()).y,0);body.add(microGroup);micro=createCrystalTensor(microGroup,4,.6);micro.update([0,0,0,0],[[0,2.7,0],[0,.9,0],[0,-.9,0],[0,-2.7,0]],{focus:0});const mb=new T.Box3().setFromObject(microGroup);box.union(mb);fieldBounds.set('live-calculation',mb);microFlow=createArrowStream(microGroup,'#b6e3ef',.12,3);microInput=createArrowStream(body,'#b6e3ef',.12,2);microOutput=createArrowStream(body,'#b6e3ef',.12,2);
   }else{
    ...
    let side=Math.max(4,x+2,...[calc.products,calc.prefix,calc.sums].filter((t):t is Tensor=>!!t).map(t=>tensorLayout(t).width/2+2)),py=gap*2;
    for(const t of parameters){const l=tensorLayout(t),isBias=step.kind==='Linear'&&step.inputs[2]?.id===t.id;box.union(field(t,new T.Vector3(side+l.width/2,isBias?-gap:py,0)));if(!isBias)py-=Math.max(4,l.depth*.65+2)}
    let outputY=0;
  ...
  const focusStage=()=>{manualFrame=undefined;const p=latest.current;let bounds=new T.Box3();const targets=p.focus?p.run.steps.filter(s=>s.group===p.focus):[];for(const s of targets){const b=fieldBounds.get(s.output.id);if(b)bounds.union(b)}if(bounds.isEmpty())for(const b of fieldBounds.values())bounds.union(b);if(p.focus&&microGroup){microGroup.position.set(bounds.max.x+3,bounds.getCenter(new T.Vector3()).y,0);microGroup.updateMatrixWorld(true);const mb=new T.Box3().setFromObject(microGroup);fieldBounds.set('live-calculation',mb);bounds.union(mb)}if(!bounds.isEmpty()){bounds.getCenter(center);bounds.getSize(extent);extent.addScalar(5);fit()}};
  ...
  const tick=(now:number)=>{raf=requestAnimationFrame(tick);const p=latest.current;if(run!==p.run||scope!==p.scope){run=p.run;scope=p.scope;rebuild()}
   ...
   if(p.scope==='root'||p.scope.startsWith('group:')||p.scope.startsWith('layer:')){
    ...
    if(active){for(const stream of connections)stream.group.visible=!stream.group.userData.skip||stream.group.userData.to===active.output.id;const local=activeHighlight?phase:cursor%1,index=Math.min(p.index,active.output.values.length-1),terms=active.trace(index),j=Math.min(terms.length-1,Math.floor(local*terms.length)),term=terms[j];for(const f of fields){const indices=terms.filter(t=>t.tensor===f.tensor.id&&t.index>=0).map(t=>t.index);f.crystals.update(f.tensor.values,f.positions,{active:indices,focus:f.tensor.id===active.output.id?index:term?.tensor===f.tensor.id?term.index:-1})}el.dataset.activeLayer=active.id;
     if(microGroup){microGroup.visible=!!term&&term.factor!==undefined;microInput!.group.visible=microGroup.visible;microOutput!.group.visible=microGroup.visible}if(micro&&term&&term.factor!==undefined){const product=term.value*(term.factor??1),prefix=terms.slice(0,j+1).reduce((n,t)=>n+t.value*(t.factor??1),0);micro.update([term.value,term.factor??active.output.values[index],product,term.factor===undefined?active.output.values[index]:prefix],[[0,2.7,0],[0,.9,0],[0,-.9,0],[0,-2.7,0]],{focus:Math.floor(local*4)%4});microFlow!.update([[0,2.7,0],[0,.9,0],[0,-.9,0],[0,-2.7,0]],local);const source=fields.find(f=>f.tensor.id===term.tensor),target=fields.find(f=>f.tensor.id===active.output.id)!;microInput!.group.visible=!!source&&term.index>=0;if(source&&term.index>=0)microInput!.update([source.group.localToWorld(new T.Vector3(...source.positions[term.index])).toArray() as Position3,microGroup!.localToWorld(new T.Vector3(0,2.7,0)).toArray() as Position3],local);microOutput!.update([microGroup!.localToWorld(new T.Vector3(0,-2.7,0)).toArray() as Position3,target.group.localToWorld(new T.Vector3(...target.positions[index])).toArray() as Position3],local);}
     if(now-lastLive>120){lastLive=now;setLive({title:active.title,equation:scalarEquation(active,index,local),step:active.id})}
    }
   ...
   if(step){const index=Math.min(p.index,step.output.values.length-1),terms=step.trace(index),current=Math.min(terms.length-1,Math.floor(phase*terms.length)),term=terms[current],out=fields.find(f=>f.tensor.id===step.output.id)!,target=out.group.localToWorld(new T.Vector3(...out.positions[index]))
    ...
    if(products&&calc.products){products.tensor.values=calc.products.values;products.crystals.update(products.tensor.values,products.positions,{focus:productIndex,active:Array.from({length:Math.min(current+1,calc.width!)},(_,i)=>(calc.productOffset??0)+i)})}
    if(prefix&&calc.prefix){prefix.tensor.values=calc.prefix.values;prefix.crystals.update(prefix.tensor.values,prefix.positions,{focus:calc.elementwise?index:Math.min(current,calc.width!-1),active:Array.from({length:Math.min(current+1,calc.width!)},(_,i)=>i)})}
    if(sums)sums.crystals.update(sums.tensor.values,sums.positions,{focus:index});
    ...
    if(now-lastLive>120){lastLive=now;setLive({title:step.title,equation:scalarEquation(step,index,phase),step:step.id})}
    el.dataset.term=String(current);el.dataset.output=String(step.output.values[index])
   }
   const highlighted=p.run.steps.find(s=>s.id===activeHighlight)?.output.id;for(const f of fields){for(const child of f.group.children)if(child instanceof T.LineSegments&&(child.material instanceof T.LineBasicMaterial)){child.material.opacity=f.tensor.id===highlighted?1:.48;child.material.color.set(f.tensor.id===highlighted?'#effaff':f.tensor.parameter?'#ddb889':'#76bdcf')}}
   el.dataset.distance=camera.position.distanceTo(controls.target).toFixed(4);renderer.render(scene,camera);const used:{x:number;y:number;w:number;h:number}[]=[];
   const ordered=[...tags].sort((a,b)=>(b.priority+(b.scope.replace('layer:','')===activeHighlight?200:0))-(a.priority+(a.scope.replace('layer:','')===activeHighlight?200:0)));
   for(const tag of ordered){const q=tag.point.clone().project(camera),anchorX=(q.x+1)*el.clientWidth/2,anchorY=(1-q.y)*el.clientHeight/2,w=tag.el.offsetWidth||150,h=tag.el.offsetHeight||42;let x=anchorX,y=anchorY,show=false;
    if((!(p.scope==='root'||p.scope.startsWith('group:')||p.scope.startsWith('layer:'))||p.showLabels||fields.length<=6||tag.priority>0||tag.scope.replace('layer:','')===activeHighlight)&&q.z<1&&q.z>0&&((!p.focus&&tag.priority>0)||tag.scope.replace('layer:','')===activeHighlight||(anchorX>-10&&anchorX<el.clientWidth+10&&anchorY>0&&anchorY<el.clientHeight))){
    ...
    tag.el.style.visibility=show?'visible':'hidden';tag.el.tabIndex=show?0:-1;tag.el.style.left=`${x}px`;tag.el.style.top=`${y}px`;tag.el.classList.toggle('is-linked',tag.scope.replace('layer:','')===activeHighlight);tag.line.style.visibility=show?'visible':'hidden';if(show){used.push({x,y,w,h});tag.line.setAttribute('x1',String(anchorX));tag.line.setAttribute('y1',String(anchorY));tag.line.setAttribute('x2',String(x));tag.line.setAttribute('y2',String(y));}
   }
   dirty=false
