...
export default function WorkScene(props:Props){const viewAction=useRef<(action:string,value?:number)=>void>(()=>{}),[following,setFollowing]=useState(false),[stageCount,setStageCount]=useState(1),[stageCursor,setStageCursor]=useState(0);const host=useRef<HTMLDivElement>(null),latest=useRef(props);latest.current=props;const [error,setError]=useState(''),[hover,setHover]=useState(''),[live,setLive]=useState({title:'',equation:'',step:''})
 useEffect(()=>{
  ...
  const activateHover=(owner:string,region:string)=>{
   ...
   hoveredRegion=region;hoverOwner=owner;
  ...
  const field=(t:Tensor,origin:T.Vector3,select?:string,title?:string,formula?:string)=>{
   ...
   const b=t.window?new T.Box3().setFromPoints(positions.map(p=>new T.Vector3(...p).add(origin))).expandByScalar(.22):new T.Box3().setFromObject(group);fieldBounds.set(t.id,b.clone());
   label(`${title??t.name}${layout.wrapped?' · 连续索引折行':''}\n[${t.window?.shapeLabel??t.shape.join(' × ')}]${t.window?' · '+t.window.label:''}`,new T.Vector3(b.max.x+.35,origin.y+.4,origin.z),select??latest.current.scope,t.id===latest.current.run.input.id||t.id===(latest.current.run.steps.find(s=>s.id===latest.current.scope)?.output.id??latest.current.run.output.id)?100:0,formula);tags.at(-1)!.el.dataset.tensor=t.id;return b
  }
  ...
  const rebuild=()=>{
   ...
   if(p.run.sourceGraph)layoutSourceScaffold(graph,p.run);else stratifyScaffold(graph);
   ...
   if(p.run.sourceGraph){
    ...
    for(const f of fields){f.positions.forEach((v,index)=>{positions.push([v[0]+f.group.position.x,v[1]+f.group.position.y,v[2]+f.group.position.z]);values.push(f.tensor.parameter?-.8:.8);sourceCells.push({field:f,index})});const b=fieldBounds.get(f.tensor.id)!;const v=[[b.min.x,b.min.y,b.min.z],[b.max.x,b.min.y,b.min.z],[b.max.x,b.min.y,b.max.z],[b.min.x,b.min.y,b.max.z]];for(let j=0;j<4;j++)framePoints.push(...v[j],...v[(j+1)%4]);
    ...
    sourcePool=createCrystalTensor(body,positions.length,.36,{valueEdges:true,bodyOpacity:.18,edgeOpacity:.5});sourcePool.update(values,positions,-1);sourceHighlights=createCrystalTensor(body,4096,.38,{valueEdges:true,bodyOpacity:.65,edgeOpacity:.95});
    sourcePool.body.userData.sourcePool=true;clicks=[sourcePool.body];
    const geometry=new T.BufferGeometry();geometry.setAttribute('position',new T.Float32BufferAttribute(framePoints,3));body.add(new T.LineSegments(geometry,new T.LineBasicMaterial({color:'#76bdcf',transparent:true,opacity:.3})));
    ...
    const path:T.Vector3[]=[];for(const edge of graph.edges){const a=fieldMap.get(edge.from),b=fieldMap.get(edge.to);if(a&&b&&!a.tensor.parameter)path.push(a.group.position.clone(),b.group.position.clone())}
    body.add(new T.LineSegments(new T.BufferGeometry().setFromPoints(path),new T.LineBasicMaterial({color:'#75a9ba',transparent:true,opacity:.14})));
   }
   flow=createArrowStream(body,'#e8f8ff',.12,2);flow.group.visible=false;
   follow=!!p.run.sourceGraph||graph.levels>80;framedOwner='';setFollowing(follow);setStageCount(visibleSteps.length);el.dataset.layers=String(graph.nodes.length);el.dataset.levels=String(graph.levels);el.dataset.mathStages=String(visibleSteps.length);el.dataset.weightEdges=String(selected.filter(s=>s.kind==='Linear').reduce((n,s)=>n+s.output.values.length*s.inputs[0].shape.at(-1)!,0));
  ...
  const projectedOwner=(event:PointerEvent)=>{
   ...
   for(const f of fields){const owner=String(f.group.userData.scope??'');if(!ownerRegion.has(owner))continue;
    ...
    const dx=Math.max(left-x,0,x-right),dy=Math.max(top-y,0,y-bottom),d=Math.hypot(dx,dy)+Math.abs(y-(top+bottom)/2)*.08;
  ...
  const tick=(now:number)=>{raf=requestAnimationFrame(tick);const p=latest.current;if(run!==p.run||!p.focusOnly&&(scope!==p.scope||layoutFocus!==p.focus)){run=p.run;scope=p.scope;layoutFocus=p.focus;liveStep='';rebuild()}
   ...
   if(active){
    ...
    if(p.run.sourceGraph&&sourceHighlights){const positions:Position3[]=[],values:number[]=[];for(const [id,state] of selections){const f=fieldMap.get(id);if(!f)continue;for(const i of new Set([...state.active,...state.focus!==undefined?[state.focus]:[]])){const q=point(id,i);if(q){positions.push(q);values.push(f.tensor.parameter?-.9:.9)}}}sourceHighlights.update(values,positions,{active:positions.map((_,i)=>i)});}
    ...
    if(now-lastLive>100){lastLive=now;setStageCursor(visibleSteps.indexOf(active));const format=(v:number)=>Number.isFinite(v)?v.toPrecision(4):String(v);setLive({title:active.title,equation:active.output.window?`${active.output.name}[${active.output.window.coordinates[index]?.join(', ')}] ← ${terms.filter(t=>t.index>=0).length} 条窗口内依赖 · 完整形状 ${active.output.window.shapeLabel}`:active.kind==='Linear'?scalarEquation(active,index,local):`输出 [${coords(outputIndex,block.source.output.shape).join(', ')}] · ${current?`读取 ${format(current.value)}${current.factor!==undefined?' × '+format(current.factor):''} → `:''}${format(active.output.values[index])}${block.streamed?' · 逐输出流式演算':''}`,step:owner})}
   }
   ...
   for(const h of holograms){h.group.visible=p.labelMode==='hover'&&(p.run.sourceGraph?ownerRegion.get(h.owner)===ownerRegion.get(liveStep):(!selective||!h.owner||h.owner===liveStep||p.run.steps.find(s=>s.id===h.owner)?.group===p.run.steps.find(s=>s.id===liveStep)?.group));if(h.group.visible)sizeHolographicLabel(h.group,camera,el.clientHeight,el.clientWidth<600?145:220);}
   el.dataset.fields=JSON.stringify(fields.flatMap(f=>{const q=f.group.position.clone().project(camera);return q.z>0&&q.z<1?[{id:f.tensor.id,owner:f.group.userData.scope,x:(q.x+1)*el.clientWidth/2,y:(1-q.y)*el.clientHeight/2}]:[]}));
