...
export function executeGenerative(id:string,c:Config,custom?:number[]):Run{
 ...
 if(id==='cut-generator'){
  ...
  const condition=()=>{group('时间条件');const requested=id==='diffusion-time'?c.dim:c.out,dim=requested,half=Math.floor(dim/2),tt=id==='diffusion-time'?e.input:e.tensor('时间步 t',[c.batch,1],Array(c.batch).fill(t));if(dim<4)throw Error('时间嵌入宽度至少为4，以定义官方指数频率间隔。');const embed=e.record('正弦余弦时间编码','Identity',[tt],e.tensor('时间编码',[c.batch,dim],Array.from({length:c.batch*dim},(_,i)=>{const j=i%dim,f=Math.exp(-Math.log(10000)*(j%half)/(half-1));return j>=half*2?0:j<half?Math.sin(tt.values[Math.floor(i/dim)]*f):Math.cos(tt.values[Math.floor(i/dim)]*f)})),R`h=\lfloor d/2\rfloor,\quad\omega_i=10000^{-i/(h-1)},\quad e_t=[\sin(t\omega),\cos(t\omega),0_{d\bmod2}]`,'指数频率与 sin/cos 两半拼接；奇数宽度末尾补零，对应官方实现。时间输入相同的批次共享编码。',i=>[e.term(tt,Math.floor(i/dim))],{operation:'time-embedding',dim});return e.linear(e.activation(e.linear(embed,c.out*4),'SiLU'),c.out*4)}
