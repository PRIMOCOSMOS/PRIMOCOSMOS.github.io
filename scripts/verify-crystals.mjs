...
await build({stdin:{contents:"export * from './src/components/mmhvae/crystalPrimitives'; export * from './src/components/mmhvae/mathVisuals';",resolveDir:process.cwd()},bundle:true,platform:'node',format:'esm',packages:'external',outfile:'tmp/crystal-verification.mjs'})
const {createCrystalTensor,createArrowStream,makeMathVisual}=await import('../tmp/crystal-verification.mjs')
...
tensor.update([0,0,0],positions,-1);assert.equal(tensor.lit.visible,false);assert.equal(tensor.litEdge.visible,false);assert.equal(tensor.body.visible,true)
