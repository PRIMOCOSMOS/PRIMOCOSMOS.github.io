...
The workstation uses the local Chinese sans stack for its compact operating UI, Tektur for module titles, and KaTeX for mathematical expressions. Body copy follows the workstation-body token; the implemented title hierarchy is 26px page title, 23px module title and 17px mathematical section title. The label selector has three modes: hover, show all and hide all. Hover reveals only the exact pointed tensor/module, not every item in its group; labels and leader lines follow the same visibility state. Scene labels retain mathematical notation; Linear labels contain KaTeX and full formulas remain in the inspector below. Wrapping labels and their leader lines keep coordinates attached to their tensors.
...
The operating bench uses a 210px catalog, flexible scene and 260px parameter column with 24px gaps inside a 1920px maximum shell. At 1250px the parameters move below the workspace; at 760px the catalog and parameters become explicit disclosures, with open parameters before the viewer. The operator index is initially collapsed and pages six operators per stage. MLP and Linear expose a quick parameter rail above the canvas, including in fullscreen: Batch, input/output features and, for MLP, hidden width and layer count. The sidebar controls remain available. The camera keeps its vertical axis upright with a more level viewing angle; complex scenes grow from 580px to 1100px according to graph depth.

**The Actual Backbone Rule.** Every scope preserves the actual dependency graph. Dense MLP input, hidden and output neuron layers stay visible with every weight edge; Linear and activation source layers are retained rather than replaced by arrays of intermediate arithmetic. Other operators retain their concrete mathematical stages. Inputs and auxiliary parameters sit near consumers, bias beside its output and convolution zero padding beside its input. Stratification follows topological depth, then separates role, operation, shape and parameter consumer into independent planes without changing real dependency edges. Unlike steps or data never share a plane. Independent branches move nearer their consumers; Linear projections stay immediately after the encoder so the μ/log-variance heads retain their proper position. Graphs deeper than 24 levels enable camera following while retaining the entire graph; mathematical navigation and full-graph fitting remain available.
...
**The Arithmetic In Place Rule.** A dense weight is represented one-to-one by a visible connection, with the complete numerical weight tensor inspectable below. Activation functions use per-coordinate f(x) transfer traces in aligned lanes, actual formulas and moving crystal units that preserve each coordinate between input and output. These traces use the implemented activation mathematics, including the GELU approximation. Scalar exponential, square and negative-log operations reuse this numerical-transfer convention. Transfer curves cover the actual inputs and at least [−6, 6] (ReLU6: [−8, 8]); 64 segments and faint axes share the same coordinate mapping as each actual (xᵢ, f(xᵢ)) point. Mapping the true function-output range to readable vertical height changes display coordinates only, never computation values. Convolution and large reductions retain concrete contributions, partial sums and persistent inter-stage links; the main dependency trunk remains intact.
...
**The Stage Boundary Rule.** The label selector has three modes: hover, show all and hide all. Hover reveals only the exact pointed tensor/module, not every item in its group; labels and leader lines follow the same visibility state. Boundary tensors participate in camera fitting regardless of label mode. Restore full graph returns to the complete computation graph in the same canvas.
...
### 文献总览造型与数值颜色（2026-09-23）

总览沿用 MMHVAE 的语义微缩模型：编码残差层片、生成解码、Gaussian 钟形、PoE 汇合盘、采样晶体、尺度变化与注意力各有轮廓。颜色在总览只编码模态/分支和功能类别；iUS、T2、ceT1、FLAIR 保持原有配色。旧造型合并为两个静态几何批次，只单独强调当前模块，不能恢复逐模块重复绘制。

数学层使用与工作站相同的 numericColor / numericPalette 和逐张量窗口最大绝对值归一化，正负色阶、零值、权重连线及高亮语义一致。未知源张量值为 NaN，显示独立的暗灰未知色，不以正数、负数或零替代。源码明确的 zeros/ones 保留真实常量。可导出当前坐标模板并导入实际激活/权重数值；导入必须校验文献、张量标识、坐标范围、重复地址和有限数值。缺失坐标保持未知，不伪造推理结果。导入只保存在当前页面内存中。
